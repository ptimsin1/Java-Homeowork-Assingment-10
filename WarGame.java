...
public class WarGame
{
   ...
   public WarGame()
   {
      ...
      while(human.takeTurn(answer) != -1 && computer.takeTurn()!= -1)
      {
         ...
         {
            computer.countWin(human.getHumanDeck().get(0)); //add the human card to the computer deck
            computer.countWin(computer.getPlayerDeck().get(0)); //add the computer to the comptuer deck
            computer.takeAwayLoss(); //remove the top card to computer deck after addded to bottom
            human.takeAwayLoss(); //remove the top card to human deck after it is added to bottom
